...
/** Tasas de cambio por defecto: meses históricos con su valor real, meses proyectados con el último valor histórico (proyección plana). */
function buildDefaultTasas(): Record<string, string[]> {
  const out: Record<string, string[]> = {}
  for (const { moneda } of forecastPaisesMoneda) {
    const hist = forecastTasasHistoricas[moneda] ?? []
    const last = hist[hist.length - 1] ?? 0
    out[moneda] = forecastMeses.map((_, i) => String(i < FORECAST_MES_ACTUAL ? (hist[i] ?? last) : last))
  }
  return out
}
...
export interface ForecastRound {
  ...
  selPeps: string[]
  notificar: boolean
  tasasCambio: Record<string, string[]>
}

export const forecastRoundsIniciales: ForecastRound[] = [
  {
    id: 'F1_2026',
    titulo: 'Forecast 1 — 2026',
    descripcion: '',
    tipo: 'global',
    estado: 'Cerrado',
    cerradoManual: false,
    ...
    selPeps: [],
    notificar: false,
    tasasCambio: buildDefaultTasas(),
  },
  {
    id: 'F2_2026',
    titulo: 'Forecast 2 — 2026',
    descripcion: '',
    tipo: 'global',
    estado: 'Abierto',
    cerradoManual: false,
  ...
  {
    id: 'F3_2026',
    titulo: 'Forecast Parcial — Proyectos TI',
    descripcion: 'Seguimiento acotado de proyectos de tecnología en curso',
    tipo: 'parcial',
    estado: 'Borrador',
    cerradoManual: false,
    ...
    desvio: 1.5,
    pepsCount: 4,
    selPeps: ['N4-2027-001', 'N4-2027-002', 'N4-2027-006', 'N4-2027-009'],
    notificar: false,
    tasasCambio: buildDefaultTasas(),
  },
]
