...
/** Las 6 celdas de los bloques Resumen Acumulado + Proyección Anual (Ajustes R1/P4): una columna "Total" por bloque + desvío monto/%. Reutilizado por Reales y Preliminares. */
export function ResumenCells({ totals, fmt }: { totals: ResumenTotals; fmt: (n: number) => string }) {
  return (
    <>
      <td className={cn(td, 'border-l-2 border-l-border bg-primary/[0.03] text-right font-mono font-bold tabular-nums text-primary')}>{fmt(totals.real)}</td>
      ...
      <td className={cn(td, 'border-l-2 border-l-border bg-primary/[0.03] text-right font-mono font-bold tabular-nums text-primary')}>{fmt(totals.realMasForecast)}</td>
