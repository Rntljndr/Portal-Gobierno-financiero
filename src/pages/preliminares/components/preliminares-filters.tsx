...
import type { PrelimFilterOptions, PrelimFilters } from '../lib/preliminares-filters-types'

interface PrelimFiltersPanelProps {
  open: boolean
  isN7: boolean
  filters: PrelimFilters
  options: PrelimFilterOptions
  onChange: <K extends keyof PrelimFilters>(key: K, value: PrelimFilters[K]) => void
  onClear: () => void
  activeCount: number
}
...
export function PrelimFilterToggle({ open, onToggle, activeCount }: { open: boolean; onToggle: () => void; activeCount: number }) {
  return (
    <button
      type="button"
      onClick={onToggle}
...
export function PrelimFiltersPanel({ open, isN7, filters, options, onChange, onClear, activeCount }: PrelimFiltersPanelProps) {
  if (!open) return null
    ...
    <div className="mx-8 mb-4 flex flex-col gap-3.5 rounded-xl border border-border bg-white p-4.5 shadow-[0_4px_16px_rgba(6,20,60,0.06)]">
      <div className="grid grid-cols-4 gap-3">
        ...
        <div>
          ...
          <input className={inputClass} placeholder="Buscar código..." value={filters.codigo} onChange={(e) => onChange('codigo', e.target.value)} />
        </div>
        <div>
        ...
        <div>
          ...
          <MultiSelect placeholder="Todas" options={options.cuentaContable} values={filters.cuentaContable} onChange={(v) => onChange('cuentaContable', v)} />
        </div>
        <div>
          <label className={labelClass}>Moneda</label>
          <MultiSelect placeholder="Todas" options={options.moneda} values={filters.moneda} onChange={(v) => onChange('moneda', v)} />
        </div>
        {isN7 && (
          <div>
            <label className={labelClass}>PEP N7</label>
            <MultiSelect placeholder="Todos" options={options.pep} values={filters.pep} onChange={(v) => onChange('pep', v)} />
          </div>
        )}
      </div>
